...
class Buzzer:
    ...
    def __init__(self, buzzer_pin):
        ...
        self.freq = 0
        self.duty = 0
        self.buzzer = PWM(Pin(buzzer_pin, Pin.OUT), freq=self.freq, duty=self.duty)
    ...
    def playsong(self, mysong):
        for index in range(len(mysong)):
            if (mysong[index] == "P"):
                bequiet()
            else:
                playtone(tones[mysong[index]])
            sleep(0.3)
        bequiet()

    def playsongtest(self):
        song = ["E5","G5","A5",
                "P","E5", "G5","B5","A5",
                "P","E5", "G5","A5",
                "P","G5", "E5"]
